...
void exit_Callback(const std_msgs::Int32::ConstPtr &msg)
{
	flag = msg->data;
}
...
{
	ros::init(argc, argv, "data_integation_node");
	ros::NodeHandle n;

	ros::Subscriber sub1 = n.subscribe<core_msgs::ball_position>("/position", 1000, camera_Callback);
	ros::Subscriber sub = n.subscribe<sensor_msgs::LaserScan>("/scan", 1000, lidar_Callback);
	ros::Subscriber time = n.subscribe<std_msgs::Float32>("/simTime", 10, time_Callback);
	ros::Subscriber sub_exit = n.subscribe<std_msgs::Int32>("/exit_call", 1, exit_Callback);
	...
	ros::Duration(5).sleep(); //this command is necessary for publishing
	...
	while(ball_dist <2){
		std_msgs::Float64 left_wheel_msg;
		std_msgs::Float64 right_wheel_msg;
		left_wheel_msg.data = -DEG2RAD(200);
		right_wheel_msg.data = DEG2RAD(200);
